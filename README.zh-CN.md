...
hook → MCP → skill → command 排序的连通分量；选中其一即可渲染该工作流的图与编号步骤。

图的绘制方式以「节点变多也仍然读得清」为目标：

- **按组件类型分列**，从左到右排布 —— 使用固定布局而非力导向模拟，节点永不重叠，同一个工作流每次绘制结果都一致
- **名称位于节点下方**，绘制在不透明色块上并使用高对比文字色，节点填充色和穿过的连线都不会把它冲淡
- **箭头**由「引用方」指向「被引用方」—— 实线表示内容中确有引用，虚线表示仅名称关键词相似
- **连线标签仅在悬停时显示**，因为一次性全部画出会淹没复杂的图；悬停时还会淡化该节点邻域以外的部分
